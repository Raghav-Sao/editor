import green from 'assests/images/business-card/dark-geen-verticle.png';
import orange from 'assests/images/business-card/orange-sample-verticle.png';
const data = {
    page: {
        w: 1002,
        h: 588
    },
    sidebar: {
        w: 100
    }
}

export const pages = {
    page_123: {
        id: 'page_123',
        name: 'font page',
        background: {
            backgroundType: 'IMAGE',
            backgroundImage: '/static/media/dark-geen-verticle.2ebc50883d1827c2723f.png',
            style: {
                height: 'auto'
            }
        },
        stickers: {
            sticker_123123: {
              _id: 'sticker_123123',
              id: 'sticker_123123',
              type: 'TEXT_STICKER',
              resource: 'Raghavendra Sao',
              styles: {
                position: {
                  left: -50,
                  top: 159
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 447,
                  translateY: -84
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 21,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 221
              },
              boundingRect: {
                l: 398,
                t: 76,
                r: 969,
                b: 118
              },
              labels: [
                'Sample text'
              ],
              name: 'sample text'
            },
            '1700909471722': {
              _id: 1700909471722,
              id: 1700909471722,
              type: 'TEXT_STICKER',
              resource: 'Senior Software Engineer',
              styles: {
                position: {
                  left: 0,
                  top: 29
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 397,
                  translateY: 74
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 18,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 270
              },
              boundingRect: {
                l: 398,
                t: 105,
                r: 1018,
                b: 147
              }
            },
            '1700909577318': {
              _id: 1700909577318,
              id: 1700909577318,
              type: 'TEXT_STICKER',
              resource: 'saoraghavendra@gmail.com',
              styles: {
                position: {
                  left: 0,
                  top: 173
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 439,
                  translateY: -19
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 15,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 244
              },
              boundingRect: {
                l: 441,
                t: 155,
                r: 1035,
                b: 197
              }
            },
            '1700909642994': {
              _id: 1700909642994,
              id: 1700909642994,
              type: 'SVG_STICKER',
              resource: '<svg viewBox="0 0 24 24" height="400" width="400"  xmlns="http://www.w3.org/2000/svg">\n        <path xmlns="http://www.w3.org/2000/svg" d="M12 2C7.6 2 4 5.6 4 10C4 15.4 11 21.5 11.3 21.8C11.5 21.9 11.8 22 12 22C12.2 22 12.5 21.9 12.7 21.8C13 21.5 20 15.4 20 10C20 5.6 16.4 2 12 2ZM12 19.7C9.9 17.7 6 13.4 6 10C6 6.7 8.7 4 12 4C15.3 4 18 6.7 18 10C18 13.3 14.1 17.7 12 19.7ZM12 6C9.8 6 8 7.8 8 10C8 12.2 9.8 14 12 14C14.2 14 16 12.2 16 10C16 7.8 14.2 6 12 6ZM12 12C10.9 12 10 11.1 10 10C10 8.9 10.9 8 12 8C13.1 8 14 8.9 14 10C14 11.1 13.1 12 12 12Z" fill="#0D0D0D"></path>\n    </svg>',
              styles: {
                position: {
                  left: -0.025083890395405595,
                  top: 277.9910542284141
                },
                translate: {
                  translateX: 396,
                  translateY: -75
                },
                rotation: {
                  rotation: 1,
                  unit: 'deg'
                },
                width: 46,
                color: 'rgb(32, 198, 161)'
              },
              boundingRect: {
                l: 398,
                t: 204,
                r: 795,
                b: 255
              }
            },
            '1700909672515': {
              _id: 1700909672515,
              id: 1700909672515,
              type: 'TEXT_STICKER',
              resource: 'Khalifa city, Abudhabi',
              styles: {
                position: {
                  left: 0,
                  top: 267
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 443,
                  translateY: -64
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 15,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 203
              },
              boundingRect: {
                l: 444,
                t: 205,
                r: 997,
                b: 247
              }
            },
            '1700909863131': {
              _id: 1700909863131,
              id: 1700909863131,
              type: 'TEXT_STICKER',
              resource: '+971 5256 12097',
              styles: {
                position: {
                  left: 0,
                  top: 352
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 442,
                  translateY: -104
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 15,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 178
              },
              boundingRect: {
                l: 443,
                t: 249,
                r: 971,
                b: 291
              }
            },
            '1700909880877': {
              _id: 1700909880877,
              id: 1700909880877,
              type: 'TEXT_STICKER',
              resource: '+91 7899 433 557',
              styles: {
                position: {
                  left: 0,
                  top: 348
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 443,
                  translateY: -73
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 15,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 190
              },
              boundingRect: {
                l: 444,
                t: 276,
                r: 984,
                b: 318
              }
            },
            '1700909974934': {
              _id: 1700909974934,
              id: 1700909974934,
              type: 'TEXT_STICKER',
              resource: 'Java Script, HTML5, CSS3,',
              styles: {
                position: {
                  left: 0,
                  top: 267
                },
                scale: 1,
                rotation: {
                  rotation: 1,
                  unit: 'deg'
                },
                translate: {
                  translateX: 56,
                  translateY: -116
                },
                color: 'rgb(74, 74, 74)',
                fontSize: 9,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 342
              },
              boundingRect: {
                l: 57,
                t: 149,
                r: 749,
                b: 197
              }
            },
            '1700910057143': {
              _id: 1700910057143,
              id: 1700910057143,
              type: 'SVG_STICKER',
              resource: '<svg viewBox="0 0 24 24" height="400" width="400"  xmlns="http://www.w3.org/2000/svg">\n        <path xmlns="http://www.w3.org/2000/svg" d="M2 5C2 3.89543 2.89543 3 4 3H20C21.1046 3 22 3.89543 22 5V16C22 17.1046 21.1046 18 20 18H4C2.89543 18 2 17.1046 2 16V5ZM20 5H4V16H20V5Z" fill="#0D0D0D"></path>\n        <path xmlns="http://www.w3.org/2000/svg" d="M22 20C22 20.5523 21.5523 21 21 21H3C2.44772 21 2 20.5523 2 20C2 19.4477 2.44772 19 3 19L21 19C21.5523 19 22 19.4477 22 20Z" fill="#0D0D0D"></path>\n    </svg>',
              styles: {
                position: {
                  left: 0,
                  top: 86
                },
                translate: {
                  translateX: 18,
                  translateY: 11
                },
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                width: 209,
                color: 'rgb(74, 74, 74)'
              },
              boundingRect: {
                l: 19,
                t: 98,
                r: 578,
                b: 312
              }
            },
            '1700910121944': {
              _id: 1700910121944,
              id: 1700910121944,
              type: 'TEXT_STICKER',
              resource: 'Project based on',
              styles: {
                position: {
                  left: 0,
                  top: 29
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 49,
                  translateY: 54
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 24,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 360
              },
              boundingRect: {
                l: 49,
                t: 84,
                r: 759,
                b: 126
              }
            },
            '1700924184787': {
              _id: 1700924184787,
              id: 1700924184787,
              type: 'SVG_STICKER',
              resource: '<svg viewBox="0 0 24 24" height="400" width="400"  xmlns="http://www.w3.org/2000/svg">\n        <path xmlns="http://www.w3.org/2000/svg" d="M3.83301 4H8.32295L9.77016 7.61803L7.44528 9.16795C7.16708 9.35342 6.99998 9.66565 6.99998 10C7.00252 10.0936 6.99998 10.001 6.99998 10.001L6.99998 10.002L6.99999 10.0043L7.00002 10.0093L7.00017 10.0217C7.00032 10.0309 7.00057 10.0421 7.00099 10.0552C7.00184 10.0814 7.00337 10.1153 7.0061 10.1564C7.01158 10.2385 7.02189 10.3494 7.04128 10.4852C7.08004 10.7565 7.15526 11.1281 7.3013 11.5662C7.59453 12.4459 8.17071 13.5849 9.29288 14.7071C10.415 15.8293 11.554 16.4054 12.4338 16.6987C12.8719 16.8447 13.2435 16.9199 13.5148 16.9587C13.6505 16.9781 13.7615 16.9884 13.8436 16.9939C13.8847 16.9966 13.9186 16.9981 13.9448 16.999C13.9579 16.9994 13.9691 16.9997 13.9783 16.9998L13.9906 17L13.9957 17L13.9979 17L13.999 17C13.999 17 14.1106 16.9939 14 17C14.3788 17 14.725 16.786 14.8944 16.4472L15.5641 15.1078L20 15.8471V20.167C17.8889 20.4723 12.1874 20.7732 7.70709 16.2929C3.22679 11.8126 3.5277 6.1111 3.83301 4ZM9.07363 10.4861L10.8796 9.28213C11.6665 8.75751 11.9784 7.75338 11.6271 6.87525L10.1799 3.25722C9.87618 2.4979 9.14076 2 8.32295 2H3.78076C2.87225 2 2.01707 2.63116 1.86792 3.6169C1.52861 5.8595 1.06677 12.481 6.29288 17.7071C11.519 22.9332 18.1405 22.4714 20.3831 22.1321C21.3688 21.9829 22 21.1277 22 20.2192V15.8471C22 14.8694 21.2932 14.0351 20.3288 13.8743L15.8929 13.135C15.0268 12.9907 14.1679 13.4281 13.7753 14.2134L13.4288 14.9064C13.3199 14.8796 13.1983 14.8453 13.0662 14.8013C12.4459 14.5946 11.5849 14.1707 10.7071 13.2929C9.82925 12.4151 9.40543 11.5541 9.19867 10.9338C9.14279 10.7662 9.10257 10.6154 9.07363 10.4861Z" fill="#0D0D0D"></path>\n    </svg>',
              styles: {
                position: {
                  left: 0,
                  top: 308
                },
                translate: {
                  translateX: 395,
                  translateY: -53
                },
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                width: 47,
                color: 'rgb(32, 198, 161)'
              },
              boundingRect: {
                l: 397,
                t: 256,
                r: 794,
                b: 308
              }
            },
            '1700924507515': {
              _id: 1700924507515,
              id: 1700924507515,
              type: 'TEXT_STICKER',
              resource: 'React js, Redux, Sass',
              styles: {
                position: {
                  left: 0,
                  top: 135
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 57,
                  translateY: 27
                },
                color: 'rgb(74, 74, 74)',
                fontSize: 9,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 251
              },
              boundingRect: {
                l: 59,
                t: 162,
                r: 660,
                b: 204
              }
            },
            '1700924686285': {
              _id: 1700924686285,
              id: 1700924686285,
              type: 'SVG_STICKER',
              resource: '<svg viewBox="0 0 24 24" height="400" width="400"  xmlns="http://www.w3.org/2000/svg">\n    <path xmlns="http://www.w3.org/2000/svg" d="M2 6C2 4.89543 2.89543 4 4 4H20C21.1046 4 22 4.89543 22 6V18C22 19.1046 21.1046 20 20 20H4C2.89543 20 2 19.1046 2 18V6ZM5.51859 6L12 11.6712L18.4814 6H5.51859ZM20 7.32877L12.6585 13.7526C12.2815 14.0825 11.7185 14.0825 11.3415 13.7526L4 7.32877V18H20V7.32877Z" fill="#0D0D0D"></path>\n        </svg>',
              styles: {
                position: {
                  left: 0,
                  top: 109
                },
                translate: {
                  translateX: 397,
                  translateY: 47
                },
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                width: 46,
                color: 'rgb(32, 198, 161)'
              },
              boundingRect: {
                l: 398,
                t: 158,
                r: 794,
                b: 209
              }
            },
            '1701538024162': {
              _id: 1701538024162,
              id: 1701538024162,
              type: 'TEXT_STICKER',
              resource: 'React Dnd',
              styles: {
                position: {
                  left: 43,
                  top: 203
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: 15,
                  translateY: -26
                },
                color: 'rgb(74, 74, 74)',
                fontSize: 9,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'left',
                fontStyle: 'normal',
                width: 199
              },
              boundingRect: {
                l: 59,
                t: 178,
                r: 608,
                b: 220
              }
            }
        },
        mappedCord: {
            horizontal: {
              '76': {
                sticker_123123: {
                  type: 'top',
                  l: 398,
                  b: 118,
                  r: 969,
                  t: 76
                }
              },
              '84': {
                '1700910121944': {
                  type: 'top',
                  l: 49,
                  b: 126,
                  r: 759,
                  t: 84
                }
              },
              '98': {
                '1700910057143': {
                  type: 'top',
                  l: 19,
                  b: 312,
                  r: 578,
                  t: 98
                }
              },
              '105': {
                '1700909471722': {
                  type: 'top',
                  l: 398,
                  b: 147,
                  r: 1018,
                  t: 105
                }
              },
              '118': {
                sticker_123123: {
                  type: 'bottom',
                  l: 398,
                  b: 118,
                  r: 969,
                  t: 76
                }
              },
              '126': {
                '1700910121944': {
                  type: 'bottom',
                  l: 49,
                  b: 126,
                  r: 759,
                  t: 84
                }
              },
              '147': {
                '1700909471722': {
                  type: 'bottom',
                  l: 398,
                  b: 147,
                  r: 1018,
                  t: 105
                }
              },
              '149': {
                '1700909974934': {
                  type: 'top',
                  l: 57,
                  b: 197,
                  r: 749,
                  t: 149
                }
              },
              '155': {
                '1700909577318': {
                  type: 'top',
                  l: 441,
                  b: 197,
                  r: 1035,
                  t: 155
                }
              },
              '158': {
                '1700924686285': {
                  type: 'top',
                  l: 398,
                  b: 209,
                  r: 794,
                  t: 158
                }
              },
              '162': {
                '1700924507515': {
                  type: 'top',
                  l: 59,
                  b: 204,
                  r: 660,
                  t: 162
                }
              },
              '178': {
                '1701538024162': {
                  type: 'top',
                  l: 59,
                  b: 220,
                  r: 608,
                  t: 178
                }
              },
              '197': {
                '1700909974934': {
                  type: 'bottom',
                  l: 57,
                  b: 197,
                  r: 749,
                  t: 149
                },
                '1700909577318': {
                  type: 'bottom',
                  l: 441,
                  b: 197,
                  r: 1035,
                  t: 155
                }
              },
              '204': {
                '1700909642994': {
                  type: 'top',
                  l: 398,
                  b: 255,
                  r: 795,
                  t: 204
                },
                '1700924507515': {
                  type: 'bottom',
                  l: 59,
                  b: 204,
                  r: 660,
                  t: 162
                }
              },
              '205': {
                '1700909672515': {
                  type: 'top',
                  l: 444,
                  b: 247,
                  r: 997,
                  t: 205
                }
              },
              '209': {
                '1700924686285': {
                  type: 'bottom',
                  l: 398,
                  b: 209,
                  r: 794,
                  t: 158
                }
              },
              '220': {
                '1701538024162': {
                  type: 'bottom',
                  l: 59,
                  b: 220,
                  r: 608,
                  t: 178
                }
              },
              '247': {
                '1700909672515': {
                  type: 'bottom',
                  l: 444,
                  b: 247,
                  r: 997,
                  t: 205
                }
              },
              '249': {
                '1700909863131': {
                  type: 'top',
                  l: 443,
                  b: 291,
                  r: 971,
                  t: 249
                }
              },
              '255': {
                '1700909642994': {
                  type: 'bottom',
                  l: 398,
                  b: 255,
                  r: 795,
                  t: 204
                }
              },
              '256': {
                '1700924184787': {
                  type: 'top',
                  l: 397,
                  b: 308,
                  r: 794,
                  t: 256
                }
              },
              '276': {
                '1700909880877': {
                  type: 'top',
                  l: 444,
                  b: 318,
                  r: 984,
                  t: 276
                }
              },
              '291': {
                '1700909863131': {
                  type: 'bottom',
                  l: 443,
                  b: 291,
                  r: 971,
                  t: 249
                }
              },
              '308': {
                '1700924184787': {
                  type: 'bottom',
                  l: 397,
                  b: 308,
                  r: 794,
                  t: 256
                }
              },
              '312': {
                '1700910057143': {
                  type: 'bottom',
                  l: 19,
                  b: 312,
                  r: 578,
                  t: 98
                }
              },
              '318': {
                '1700909880877': {
                  type: 'bottom',
                  l: 444,
                  b: 318,
                  r: 984,
                  t: 276
                }
              }
            },
            vertical: {
              '19': {
                '1700910057143': {
                  type: 'left',
                  l: 19,
                  b: 312,
                  r: 578,
                  t: 98
                }
              },
              '49': {
                '1700910121944': {
                  type: 'left',
                  l: 49,
                  b: 126,
                  r: 759,
                  t: 84
                }
              },
              '57': {
                '1700909974934': {
                  type: 'left',
                  l: 57,
                  b: 197,
                  r: 749,
                  t: 149
                }
              },
              '59': {
                '1700924507515': {
                  type: 'left',
                  l: 59,
                  b: 204,
                  r: 660,
                  t: 162
                },
                '1701538024162': {
                  type: 'left',
                  l: 59,
                  b: 220,
                  r: 608,
                  t: 178
                }
              },
              '397': {
                '1700924184787': {
                  type: 'left',
                  l: 397,
                  b: 308,
                  r: 794,
                  t: 256
                }
              },
              '398': {
                '1700924686285': {
                  type: 'left',
                  l: 398,
                  b: 209,
                  r: 794,
                  t: 158
                },
                sticker_123123: {
                  type: 'left',
                  l: 398,
                  b: 118,
                  r: 969,
                  t: 76
                },
                '1700909642994': {
                  type: 'left',
                  l: 398,
                  b: 255,
                  r: 795,
                  t: 204
                },
                '1700909471722': {
                  type: 'left',
                  l: 398,
                  b: 147,
                  r: 1018,
                  t: 105
                }
              },
              '441': {
                '1700909577318': {
                  type: 'left',
                  l: 441,
                  b: 197,
                  r: 1035,
                  t: 155
                }
              },
              '443': {
                '1700909863131': {
                  type: 'left',
                  l: 443,
                  b: 291,
                  r: 971,
                  t: 249
                }
              },
              '444': {
                '1700909880877': {
                  type: 'left',
                  l: 444,
                  b: 318,
                  r: 984,
                  t: 276
                },
                '1700909672515': {
                  type: 'left',
                  l: 444,
                  b: 247,
                  r: 997,
                  t: 205
                }
              },
              '578': {
                '1700910057143': {
                  type: 'right',
                  l: 19,
                  b: 312,
                  r: 578,
                  t: 98
                }
              },
              '608': {
                '1701538024162': {
                  type: 'right',
                  l: 59,
                  b: 220,
                  r: 608,
                  t: 178
                }
              },
              '660': {
                '1700924507515': {
                  type: 'right',
                  l: 59,
                  b: 204,
                  r: 660,
                  t: 162
                }
              },
              '749': {
                '1700909974934': {
                  type: 'right',
                  l: 57,
                  b: 197,
                  r: 749,
                  t: 149
                }
              },
              '759': {
                '1700910121944': {
                  type: 'right',
                  l: 49,
                  b: 126,
                  r: 759,
                  t: 84
                }
              },
              '794': {
                '1700924184787': {
                  type: 'right',
                  l: 397,
                  b: 308,
                  r: 794,
                  t: 256
                },
                '1700924686285': {
                  type: 'right',
                  l: 398,
                  b: 209,
                  r: 794,
                  t: 158
                }
              },
              '795': {
                '1700909642994': {
                  type: 'right',
                  l: 398,
                  b: 255,
                  r: 795,
                  t: 204
                }
              },
              '969': {
                sticker_123123: {
                  type: 'right',
                  l: 398,
                  b: 118,
                  r: 969,
                  t: 76
                }
              },
              '971': {
                '1700909863131': {
                  type: 'right',
                  l: 443,
                  b: 291,
                  r: 971,
                  t: 249
                }
              },
              '984': {
                '1700909880877': {
                  type: 'right',
                  l: 444,
                  b: 318,
                  r: 984,
                  t: 276
                }
              },
              '997': {
                '1700909672515': {
                  type: 'right',
                  l: 444,
                  b: 247,
                  r: 997,
                  t: 205
                }
              },
              '1018': {
                '1700909471722': {
                  type: 'right',
                  l: 398,
                  b: 147,
                  r: 1018,
                  t: 105
                }
              },
              '1035': {
                '1700909577318': {
                  type: 'right',
                  l: 441,
                  b: 197,
                  r: 1035,
                  t: 155
                }
              }
            }
        },
        drawPoints: []
    },
    page_124: {
        id: 'page_124',
        name: 'font page',
        background: {
            backgroundType: 'IMAGE',
            backgroundImage: '/static/media/orange-sample-verticle.d7ff8b47cd6d939012cf.png',
            style: {
                height: 'auto'
            }
        },
        stickers: {
            sticker_123123: {
              _id: 'sticker_123123',
              id: 'sticker_123123',
              type: 'TEXT_STICKER',
              resource: 'Select me',
              styles: {
                position: {
                  left: 0,
                  top: 159
                },
                scale: 1,
                rotation: {
                  rotation: 0,
                  unit: 'deg'
                },
                translate: {
                  translateX: -0.14453125,
                  translateY: -21.3203125
                },
                color: 'rgb(32, 198, 161)',
                fontSize: 21,
                height: 'auto',
                fontWeight: 'normal',
                textAlign: 'center',
                fontStyle: 'normal',
                width: 288
              },
              boundingRect: {
                l: 1,
                t: 139,
                r: 289,
                b: 181
              },
              labels: [
                'Sample text'
              ],
              name: 'sample text'
            }
        },
        mappedCord: {
            horizontal: {
              '139': {
                sticker_123123: {
                  type: 'top',
                  l: 1,
                  b: 181,
                  r: 289,
                  t: 139
                }
              },
              '181': {
                sticker_123123: {
                  type: 'bottom',
                  l: 1,
                  b: 181,
                  r: 289,
                  t: 139
                }
              }
            },
            vertical: {
              '1': {
                sticker_123123: {
                  type: 'left',
                  l: 1,
                  b: 181,
                  r: 289,
                  t: 139
                }
              },
              '289': {
                sticker_123123: {
                  type: 'right',
                  l: 1,
                  b: 181,
                  r: 289,
                  t: 139
                }
              }
            }
          }
    },
};